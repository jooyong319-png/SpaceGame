...
namespace SalvageRun.Data
{
    ...
    public static class TechTreeDefaults
    {
        ...
        public static void Fill(GameContent c)
        {
            ...
            // 🔴 **네 번째 무기 — 유도 미사일** (2026-08-27 사장님: *"투척류, 미사일 같은 거"*).
            //    3구역(코어)에 둔 이유: 무기 하나가 화력을 크게 바꾸므로
            //    **구역마다 하나씩** 열려야 진행이 계단이 된다.
            //    (1구역 방전 · 2구역 원반 · 3구역 미사일)
            Wep("wep_missile", "유도 미사일", WeaponKind.Missile,
                "느리게 날아가 목표를 따라간다. 닿으면 터진다",
                -4, -3, 2200, 30, 6, "wep_discus");
            ...
            N("sal2", "자동 분류기", "크레딧 +5%", TechBranch.Salvage, 2, -1,
              // 🔴 회로를 뺐다 (2026-08-27) — 아래 `sal4`(칸 +1)로 가는 길목이라
              //    여기가 막히면 **1구역에서 수입을 늘릴 방법이 하나도 없다.**
              TechEffect.ValueMul, 0.05f, 30, 0, 0, 6, 1.55f, "sal1");

            // 🔴 레벨업이 없어져(2026-08-26) 경험치 노드가 죽었다 — 견인 쪽으로 갈아끼웠다
            N("sal3", "경량 견인줄", "짐이 덜 무겁다 (+8%)", TechBranch.Salvage, 2, -2,
              TechEffect.TowWeight, 0.08f, 34, 0, 0, 6, 1.55f, "sal1");

            // 🔴 **끌 수 있는 칸 +1.** 지금 판에서 제일 크게 체감되는 강화다 —
            //    무게 감소는 "덜 느려진다"지만 이건 **벽 자체를 민다.**
            // 🔴 **1구역에서 칸을 늘릴 수 있어야 한다** (2026-08-27 실측 뒤).
            //
            //    1구역 천장에서 재 보니 **주움 75 · 가져옴 6**이었다 —
            //    부순 것의 **92%를 버린다.** 화력을 올려도 수입이 안 늘기 때문이다:
            //    **한 판 수입 = 칸 수 × 덩어리**이고 칸은 6에 묶여 있었다.
            //    칸 노드 네 개가 전부 회로(2구역)를 요구해서 **1구역엔 수입 레버가 없었다.**
            //
            //    ⚠️ 칸을 늘리면 *"가져갈까 버릴까"*가 무뎌진다고 앞서 적었는데,
            //       실측은 반대를 말한다 — **꽉 찬 채로 보낸 시간이 91%**다.
            //       결정이 *안 켜지는* 게 아니라 **너무 켜져서 하나하나가 안 아프다.**
            //       칸을 6 → 11로 늘려도 주움이 75면 여전히 고르게 된다.
            N("sal4", "확장 견인대", "끌 수 있는 칸 +1", TechBranch.Salvage, 3, -1,
              TechEffect.TowCapacity, 1f, 88, 0, 0, 5, 2.0f, "sal2");
            ...
            // ---- 유도 미사일 (4,-3에서 아래로) ----
            Won("ms1", "확산 탄두", "미사일 폭발 반경 +14%", WeaponKind.Missile, -4, -4,
                TechEffect.WeaponRangeOne, 0.14f, 420, 12, 2, 4, 1.85f, "wep_missile");

            Won("ms2", "다연장 발사", "미사일 발사 수 +1", WeaponKind.Missile, -5, -4,
                TechEffect.WeaponCountOne, 1f, 900, 24, 5, 3, 2.1f, "ms1");

            Won("ms3", "고밀도 장약", "미사일 피해 +16%", WeaponKind.Missile, -4, -5,
                TechEffect.WeaponPowerOne, 0.16f, 520, 14, 3, 5, 1.8f, "ms1");
            ...
            // ==========================================================================
            //  🔴 **직송 드론** (2026-08-27 사장님 지시) — 인벤토리 밖에서 집으로 보낸다.
            //
            //     ⚠️ **등급으로 나눈 것이 핵심이다.** 랭크 1은 고철만, 6이면 전부.
            //        처음부터 다 가져가면 *"가져갈까 버릴까"*가 사라진다 —
            //        **값진 것은 한동안 직접 실어야 한다.**
            //     1구역(고철만)에서 열 수 있게 회로를 안 붙였다. 거기가 제일 답답한 구간이다.
            // ==========================================================================
            N("haul1", "직송 드론", "드론이 재화를 집으로 바로 보낸다 (랭크마다 한 등급씩 위로)",
              TechBranch.Salvage, 1, -2, TechEffect.HaulerGrade, 1f, 240, 0, 0, 6, 2.15f, "sal3");
